...
const $grid=$("#grid"), $search=$("#search"), $cat=$("#category"), $comp=$("#composition"), $print=$("#print");
...
function render(){
  ...
  $grid.innerHTML = list.map(card).join("") || `<div style="grid-column:1/-1;text-align:center;color:#666;padding:30px">Ничего не найдено.</div>`;
}
function card(p){
  ...
  const msg = encodeURIComponent(
    ...
    `• Цена: $${p.price_usd} / м\n• Кол-во: ___ м\n`+
    (p.images?.[0]? `Фото: ${location.origin}${p.images[0]}\n`:"")
  );
  return `
  <article class="card">
